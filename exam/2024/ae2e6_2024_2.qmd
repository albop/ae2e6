...
$$\lambda_t = \beta \lambda_{t+1} \frac{1+r}{1+\rho}$$

D'où
$$\frac{1+\rho}{1+r}=\beta \left( \frac{C_{i,t+1}}{C_{i,t1}}\right)^{-\theta}\left( \frac{Z_{i,t+1}}{Z_{i,t}} \right)^ {\theta-1}$$
...
$$\frac{1+\rho}{1+r}=\beta \left( \frac{C_{t+1}}{C_{t}}\right)^{-\theta}\left( \frac{C_{t}}{C_{t-1}} \right)^ {\phi(\theta-1)}$$

On peut la logdifferencier pour obtenir:

$$\theta \left( \log(C_{t+1}) - \log(C_t) \right) =   \phi(\theta-1) \left( \log(C_t) - \log(C_{t-1}) \right)$$
